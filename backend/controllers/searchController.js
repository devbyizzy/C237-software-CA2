...
exports.search = async (req, res) => {
  try {
    ...
    //  Table: student_groups WHERE group_type = 'interest'
    // =============================================
    if (searchGroups) {
      // COUNT query for total matching interest groups
      const countSql = `
      ...
      // Get the safe ORDER BY value from the whitelist
      const orderBy = getSort(groupSortOptions, sort, 'g.created_at DESC');

      // SELECT query for interest group data
      const dataSql = `
        SELECT
          g.group_id,
          g.group_name,
      ...
      // If showing All results, just get the count for the tab
      if (searchAll) {
        promises.push(
      ...
      // If specifically browsing Groups tab, get full paginated data
      if (selectedType === 'groups') {
        ...
        promises.push(
          runCount(countSql, countParams).then(count => { allCount = count; })
        );
      }
    }
